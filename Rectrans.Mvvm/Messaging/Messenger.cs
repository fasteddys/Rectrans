...
namespace Rectrans.Mvvm.Messaging
{
    public class Messenger : IMessenger
    {
            ...
            lock (lists)
            {
                ...
                foreach (var list in lists)
                {
                    var recipientsToRemove = list.Value
                        .Where(item => !item.Action.IsAlive)
                        .ToList();
        ...
        private static void SendToList<TMessage>(
            TMessage message,
            IEnumerable<WeakActionAndToken>? weakActionAndTokens,
            ...
            Dictionary<Type, List<WeakActionAndToken>>? lists)
        {
            ...
            lock (lists)
            {
                foreach (var messageType in lists.Keys)
                {
                    ...
                    {
                        if (item.Action is IExecuteWithObject weakAction
